...
export const PUT = withErrorBoundary(
  async (request: NextRequest, context: { params: Promise<{ appointmentId: string }> }) => {
    ...
    if (!body?.prescriber?.name) {
      return contractError("VALIDATION_ERROR", "prescriber.name is required", {
        field: "prescriber",
      });
    }

    setAppointmentPrescriber(appointment.id, body.prescriber.name, body.prescriber.rppsId ?? null);
    return ok({});
  },
);
