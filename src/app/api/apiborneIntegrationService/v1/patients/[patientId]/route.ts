...
export const PATCH = withErrorBoundary(
  async (request: NextRequest, context: { params: Promise<{ patientId: string }> }) => {
    ...
    if (!Number.isFinite(id) || !getPatient(id)) {
      return contractError("UNKNOWN_PATIENT", `Patient '${patientId}' not found`);
    }
    ...
    if (!body?.patient || typeof body.patient !== "object") {
      return contractError("VALIDATION_ERROR", "patient is required", { field: "patient" });
    }
